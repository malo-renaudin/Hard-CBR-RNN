...
def create_configs():
    ...
    configs_dir = Path("job_lstm_configs")
...
#SBATCH --output=job_outputs/job_lstm_%A_%a.out
#SBATCH --error=job_outputs/job_lstm_%A_%a.err
#SBATCH --ntasks=1
...
python grid_search_lstm.py $SLURM_ARRAY_TASK_ID
"""
    
    with open("run_array_lstm.sh", 'w') as f:
...
def train_single_job(job_id):
    ...
    job_dir = Path(f"job_lstm_{job_id:03d}")
    job_dir.mkdir(exist_ok=True)
    
    # Load config
    config_file = Path(f"job_lstm_configs/config_{job_id:03d}.json")
    ...
    try:
        ...
        # Create model
        model = SimpleLSTM_LM(**model_kwargs)
...
1. Submit the job: sbatch _lstm.sh
2. Monitor jobs: squeue -u $USER
3. Check results: python train_single.py collect

Files created:
- job_lstm_configs/config_*.json ({num_configs} config files)
- shared_data/tokenizer.json
- run_array_lstm.sh (SLURM script)
        """)
