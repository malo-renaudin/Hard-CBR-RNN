...
class UniversalDataModule(pl.LightningDataModule):
    ...
    def collate_fn(self, batch):
        ...
        target_tensor = torch.stack(
            [torch.tensor(seq, dtype=torch.long) for seq in target_ids])
...
class ModelFactory:
    ...
    def create_model(model_type: str, model_config: Dict[str, Any], vocab_size: int) -> pl.LightningModule:
        ...
        if model_type.upper() == 'CBR_RNN':
            cbr_config = {
                'ntoken': vocab_size,
                'ninp': int(model_config.get('ninp', 256)),
                'nhid': int(model_config.get('nhid', 512)),
                'nheads': int(model_config.get('nheads', 1)),
                'seq_len': int(model_config.get('seqlen', 128)),
                'compressed_dim': int(model_config.get('compressed_dim', 1)),
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        if hasattr(pl_module, 'temp_scheduler') and hasattr(pl_module, 'gumbel_softmax'):
            if pl_module.gumbel_softmax:
                ...
                print(
                    f"Epoch {trainer.current_epoch}: Temperature = {current_temp:.6f}")
...
def load_comprehensive_checkpoint(checkpoint_path: str, model: pl.LightningModule = None):
    ...
    if checkpoint.get('current_temperature') is not None:
        print(
            f"  Current temperature: {checkpoint['current_temperature']:.6f}")
    ...
    if model is not None:
        ...
        if checkpoint.get('temp_scheduler_state') and hasattr(model, 'temp_scheduler'):
            ...
            print(
                f"  Temperature scheduler restored (epoch {temp_state['current_epoch']})")
...
def train_model(config_path: str, resume_from_checkpoint: Optional[str] = None):
    ...
    if hasattr(model, 'gumbel_softmax') and model.gumbel_softmax:
        if hasattr(model, 'temp_scheduler'):
            print(
                f"Using temperature scheduler: {model.temp_scheduler.initial_temp} -> {model.temp_scheduler.final_temp}")
        else:
...
def test_model_creation(config_path: str):
    ...
    try:
        ...
        model = ModelFactory.create_model(
            model_type, model_config, test_vocab_size)
        ...
        dummy_targets = torch.randint(
            0, test_vocab_size, (seq_len, batch_size))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-only', action='store_true',
                        help='Only run model creation test')
    ...
    if args.test_only:
        ...
        resume_training_from_comprehensive_checkpoint(
            args.resume_comprehensive, args.config)
    else:
